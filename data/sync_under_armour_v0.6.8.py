...
STAGING = DATA / "staging_under_armour_v0.6.8.json"
VERSION = "0.6.8"
...
BROAD_TERMS = {
    "under armour", "アンダーアーマー", "ua", "infinite", "elite", "pro",
    "velociti", "flow", "dynamic", "tribase", "reign", "curry",
    "running", "training", "basketball", "shoes"
}
...
def update_docs() -> None:
    ...
    text = text.replace("# Kick×Kick Model Master Coverage v3.4", "# Kick×Kick Model Master Coverage v3.5")
    marker = "| TEVA | PASS 8 | PASS | PASS | PASS | PASS | 公式カタログで確認した8モデル系列を追加。色・コラボ・性別・季節柄は別モデルとして数えない。 |"
    row = marker + "\n| UNDER ARMOUR | PASS 8 | PASS | PASS | PASS | PASS | 公式日本カタログで確認した8モデル系列を追加。色・性別・サイズ・限定カラーは別モデルとして数えない。 |"
    if "| UNDER ARMOUR | PASS 8 |" not in text:
        text = text.replace(marker, row)
    text = text.replace("2. 次のTier Cブランドを1ブランド集中で追加（TEVA完了後）", "2. 次のTier Cブランドを1ブランド集中で追加（UNDER ARMOUR完了後）")
    if "2026-07-24 v0.6.8" not in text:
    ...
    text = text.replace("# Kick×Kick Task Board v3.6", "# Kick×Kick Task Board v3.7")
    text = text.replace("ACTIVE / MVP DATA PASS / MARKET REFERENCE GROWTH / DATA v0.6.7 SYNCED / TEVA PASS 8", "ACTIVE / MVP DATA PASS / MARKET REFERENCE GROWTH / DATA v0.6.8 SYNCED / UNDER ARMOUR PASS 8")
    if "## 2026-07-24 UNDER ARMOUR v0.6.8" not in text:
    ...
    append_once(DATA / "README.md", "## v0.6.8 UNDER ARMOUR", """
## v0.6.8 UNDER ARMOUR
...
""")
    append_once(DATA / "validation_rules.md", "## UNDER ARMOUR v0.6.8 audit", """
...
""")
    append_once(DATA / "CHANGELOG.md", "## 0.6.8 - 2026-07-24", """
## 0.6.8 - 2026-07-24
...
def main() -> None:
    ...
    if staging.get("brandId") != "under_armour" or not staging.get("audit", {}).get("readyForRootJsonMerge"):
        raise ValueError("UNDER ARMOUR staging is not merge-ready")
    ...
    update_docs()
    print("UNDER ARMOUR v0.6.8 sync complete")
