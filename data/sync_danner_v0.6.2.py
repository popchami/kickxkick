...
STAGING = DATA / "staging_danner_v0.6.2.json"
VERSION = "0.6.2"
...
BROAD_TERMS = {"danner", "light", "mountain", "field", "trail", "jag", "cascade", "crest", "boot", "boots", "hiking", "waterproof"}
...
def update_docs() -> None:
    ...
    text = text.replace("# Kick×Kick Model Master Coverage v2.8", "# Kick×Kick Model Master Coverage v2.9")
    marker = "| FILA | PASS 6 | PASS | PASS | PASS | PASS | 高確度6モデルを追加。ブランド・一般語単体は除外。 |"
    row = marker + "\n| Danner | PASS 6 | PASS | PASS | PASS | PASS | 高確度6モデルを追加。ブランド・一般語単体は除外。 |"
    if "| Danner | PASS 6 |" not in text:
        text = text.replace(marker, row)
    text = text.replace("2. 次のTier Cブランドを1ブランド集中で追加（FILA完了後）", "2. 次のTier Cブランドを1ブランド集中で追加（Danner完了後）")
    if "2026-07-15 v0.6.2" not in text:
    ...
    text = text.replace("# Kick×Kick Task Board v3.0", "# Kick×Kick Task Board v3.1")
    text = text.replace("ACTIVE / MVP DATA PASS / MARKET REFERENCE GROWTH / DATA v0.6.1 SYNCED / FILA PASS 6", "ACTIVE / MVP DATA PASS / MARKET REFERENCE GROWTH / DATA v0.6.2 SYNCED / DANNER PASS 6")
    if "## 2026-07-15 Danner v0.6.2" not in text:
        text = text.rstrip() + "\n\n## 2026-07-15 Danner v0.6.2\n- [x] Danner 6モデル追加\n- [x] Alias / searchKeywords高純度監査\n- [x] data / app assets同期\n- [x] README / validation / CHANGELOG / Coverage整合\n- [ ] Search MVP実動作テスト\n"
    task.write_text(text, encoding="utf-8")

    append_once(DATA / "README.md", "## v0.6.2 Danner", """
## v0.6.2 Danner
...
""")
    append_once(DATA / "validation_rules.md", "## Danner v0.6.2 audit", """
## Danner v0.6.2 audit
...
""")
    append_once(DATA / "CHANGELOG.md", "## 0.6.2 - 2026-07-15", """
## 0.6.2 - 2026-07-15
...
def main() -> None:
    ...
    if staging.get("brandId") != "danner" or not staging.get("audit", {}).get("readyForRootJsonMerge"):
        raise ValueError("Danner staging is not merge-ready")
    ...
    update_docs()
    print("Danner v0.6.2 sync complete")
